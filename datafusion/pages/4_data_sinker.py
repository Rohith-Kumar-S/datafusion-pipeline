...
if st.session_state.target_control == "Create target":
    ...
    with col1:
        st.selectbox(
            "Target type",
            options=["download", "console", "external"],
            key="target_type",
        )
    with col2:
        st.text_input("Target name", key="target_name")
    with col3:
        if st.session_state.target_type == "external":
            st.text_input(
                "Target path",
                key="target_path",
                placeholder="e.g. s3://my-bucket/my-data/",
            )
    col1, col2, col3 = st.columns(3)
    with col1:
    ...
    if st.button("Save target"):
        if st.session_state["target_name"] != "":
            ...
            if (
                "target_path" in st.session_state
                and st.session_state["target_path"] != ""
            ):
                st.session_state.targets[new_target_name]["path"] = st.session_state[
                    "target_path"
                ]
