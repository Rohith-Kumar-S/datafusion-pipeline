...
with b:
    ...
    col1, col2, col4, col3 = st.columns([4, 4, 12, 2], vertical_alignment="bottom")
    with col2:
    ...
    with col1:
        if pipeline_selection != 'Create pipeline': 
            pipeline = st.selectbox("Select pipeline", options=list(st.session_state.pipelines.keys()), index=0, key="selected_pipeline", disabled=pipeline_selection == 'Create pipeline', label_visibility="collapsed", help="Select a pipeline to load.", on_change=lambda: st.session_state.temp_pipelines.update(st.session_state.pipelines[st.session_state.selected_pipeline]) if st.session_state.selected_pipeline in st.session_state.pipelines else None)
    ...
    #             options.extend(['bleh'])
    #         if st.session_state[f'layer_type_{i}'] == 'Processor':
    #             options.extend(list(st.session_state.rules.keys()))
    #         if st.session_state[f'layer_type_{i}'] == 'Fusion':
    #             options.extend(list(st.session_state.fusions.keys()))
    #         if st.session_state[f'layer_type_{i}'] == 'Target':
